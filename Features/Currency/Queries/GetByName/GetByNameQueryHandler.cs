...
{
    public class GetByNameQueryHandler : IQueryHandler<GetByNameQuery, CurrencyResponseDto>
    {
        private readonly ICurrencyRepository _currencyRepository;

        public GetByNameQueryHandler(ICurrencyRepository currencyRepository)
        {
            _currencyRepository = currencyRepository;
        }

        public async Task<Result<CurrencyResponseDto>> Handle(GetByNameQuery query, CancellationToken cancellationToken)
        {
            try
            {
                var currency = await _currencyRepository.GetByNameAsync(query.Name);
